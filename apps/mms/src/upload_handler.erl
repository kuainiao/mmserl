    ...
    case mms_header:parse(Req) of
        #mms_headers{
            ...
            case mms_header:verify_upload(Req) of
                ...
                _ ->
                    Data = mms_header:parse_body(Req),
                    case mms_header:action(Range, FileSize) of
                        upload ->
                            Uid2 = uuid:generate(),
                            case mms_mysql:save(Uid2, FileName, Owner) of
                                {error, _} ->
                                    mms_response:bad_request(Req, Opts, <<"invalid uid">>);
                                _ ->
                                    case mms_s3:upload(Uid2, Data) of
                                        ok ->
                                            mms_response:ok(Req, Opts, Uid2);
                                        error ->
                                            mms_response:bad_request(Req, Opts, <<"upload error">>)
                                    end
                            end;
                        append_upload ->
                        ...
                        new_file ->
                            ...
                            case mms_redis:insert(<<"filename:", Uid2/binary>>, Uid2) of
                                ok ->
                                    ...
                                    mms_response:ok(Req, Opts, Uid2);
                                _ ->
                                    mms_response:bad_request(Req, Opts, <<"failed">>)
                            end;
                        append_file ->
                            case mms_redis:get(<<"filename:", Uid/binary>>) of
                                ok ->
